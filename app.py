...
# Special cases where  code is not obvious
_SPECIAL = {
    "RAJESH.BO": "544291",      # Rajesh Power Services (BSE numeric code)
    # add more overrides here if needed
}

def ticker_to__code(symbol):
    """
    Convert a yfinance-style ticker to  company code.
...
def _base(code):
    return f"https://www..in/company/{code}/"

@lru_cache(maxsize=128)
def fetch__html(symbol):
    """
    Download  consolidated page HTML for a symbol.
    Cached to avoid repeated network calls.
    """
    code = ticker_to__code(symbol)
    if not code:
        return None
    url = _base(code) + "consolidated/"
...
# =========================================================
# Fundamentals (key metrics + YoY) from 
# =========================================================

@lru_cache(maxsize=128)
def fetch__metrics(symbol):
    """
    ...
    code = ticker_to__code(symbol)
    links = {
        " Company Page": _base(code) if code else None,
        " Balance Sheet": (_base(code) + "consolidated/#balance-sheet") if code else None,
    }
    ...
    return {
        ...
        " Company Page": links.get(" Company Page"),
        " Balance Sheet": links.get(" Balance Sheet"),
    }
...
def update_dashboard(selected_symbols):
    ...
    for symbol in selected_symbols:
        ...
        fundamentals = fetch__metrics(symbol)
        ...
        if ann_list:
            ...
            ann_ui = html.Div(
                "No recent announcements found on .",
